...
function search_candidate($type,$field,$string)
	{
        ...
        $field_array=array('a.email','b.first_name','b.middel_name','b.last_name','b.city','b.country', 'b.state', 'b.phone_no', 'b.postal_code','b.employment_type',
					 'b.salary', 'b.hourly_rate', 'b.work_authorization', 'b.travel', 'b.experience', 'b.position','c.education', 'c.institution', 'c.city', 'c.state', 'c.education',
...
//	}
  ...
  return $where.") AND ";
	...
	function cat_list($p_cid=0,$space='',$sel_cat){
 	$q="SELECT * FROM sedna_category WHERE parent_cid='$p_cid' and status=0";
	...
	if($count > 0){
		...
		while($row=mysql_fetch_array($r)){
			...
			echo "<option  ".$selected." value= '".$row['cid']."'>".$space."&nbsp;".$row['name']."</option>";
			
			$this->cat_list($row['cid'],$space,$sel_cat);
		}
