...
{  
    ...
    $sql = "SELECT * FROM sedna_operator_request_consultants_reg WHERE email='$email'";
  	$results = mysqli_query($conn, $sql);
  	if (mysqli_num_rows($results) > 0) {
  	 echo "<script>alert('Email Already Registered');</script>";
  	 echo "<script>location.href = '../?page=register_now';</script>";
  	  	exit();
  	}else{
  	  echo '';
  	}
...
$insert = $conn->query("insert into sebna_profile_tbl (ssn_num,aadhar_num,source,address,profile_image,language_know,reg_id,first_name,middel_name,last_name,email,city,contact,Country,state,postal_code,you_are,employment_type,work_authorization,current,employer_name,employer_company,employer_email,employer_number,last_used,reg_date,preferred_location,travel,experience,position,salary,searchable,relocate,security,hourly_rate,resume_data,comment,add_by,zipcode)
           values('$ssn_num','$aadhar_num','public','$remark','$new','$language','$reg_id','$first_name','$middel_name','$last_name','$email','$city','$contact','$Country','$state','$postal_code','$you_are','$employment_type','$work_authorization','$current_date','$employer_name','$employer_company','$employer_email','$employer_number','$last_used','$reg_date','$preferred_location','$travel','$experience','$position','$salary','$searchable','$relocate','$security','$hourly_rate','$skill_pro_resume','','','$zipcode')"); 
...
$conn->query("INSERT INTO `sedna_user_registration_tbl`(`master_id`, `name`, `contact`, `email`, `address`, `department`, `TL_id`, `user_pass`, `base_password`, `joining_date`, `status`, `delete_status`, `emp_name`, `industry`, `web`, `who_we`, `location`, `ATs`, `profile_img`,alfakey,video_id,file_name,file_url) VALUES
...
$message.='Please find below login credentials And complete your profile now, <a href="https://itcareer.app/candidate-login/?page=login">click here</a> to login.<br><br>';
...
$message.='<b>Team TechSimba</b><br>';
			$this_object->SendMailToAdmin_new($to,$message,$subject);
