...
//ini_set('display_startup_errors', TRUE);
		...
		//echo "<pre>";print_r($_POST);
		 $keyword_set=mysql_real_escape_string($_POST["keyword_set"]);
		 $keyword_string=mysql_real_escape_string($_POST["keyword_search"]);
        $_SESSION['string_key_word']=$keyword_string;
        $_SESSION['string_key']=$keyword_set;
        
        $search_string_full.=$keyword_string;
		
		
		$company_set=mysql_real_escape_string($_POST["company_set"]);
		$company_name=mysql_real_escape_string($_POST["company_name"]);
		
		$job_title_set=mysql_real_escape_string($_POST["job_title_set"]);
		$work_feild=mysql_real_escape_string($_POST["work_feild"]);
		
		$skill_set=mysql_real_escape_string($_POST["skill_set"]);
		$skill_name=$_POST["skill_name"];
		$experince=$_POST["experince"];
		
		$work_country=mysql_real_escape_string($_POST['work_country']);
        $search_string_full.= $work_country;
        
		$selected_state=mysql_real_escape_string($_POST['state']);
		$last_active_days=mysql_real_escape_string($_POST['last_active_days']);
		$employment_type=mysql_real_escape_string($_POST['employment_type']);
		$work_authorization=mysql_real_escape_string($_POST['work_authorization']);
		$travel_preferences=mysql_real_escape_string($_POST['travel_preferences']);
		$willing_to_relocate=mysql_real_escape_string($_POST['willing_to_relocate']);
		$year_experience=mysql_real_escape_string($_POST['year_experience']);
		$degree=mysql_real_escape_string($_POST['degree']);
		$security_clearance=mysql_real_escape_string($_POST['security_clearance']);
		$hourly_salary=mysql_real_escape_string($_POST['hourly_salary']);
		$annual_salary=mysql_real_escape_string($_POST['annual_salary']);
		
		$experince=$_POST['experince'];
		$distance_mi=mysql_real_escape_string($_POST['distance_mi']);
...
//        }
		...
		$select_city_zip=mysql_query("select * from zip_code where city like '%". $work_country . "%'");
		$fetch_city_zip=mysql_fetch_array($select_city_zip);
		...
		$where_condition.=$qu_employment_where.") AND ";
		}
		...
		$where_condition.=$work_authorization_wt_where." AND ";
		...
		$where_condition.=$salary_qu_where.") AND ";
		}
		...
		$where_condition.=$salary_qu_hourly_where.")AND ";
		...
		$where_condition.=$travel_preferences_tp_where." AND ";
		...
		$willing_to_relocate_where.="(b.relocate LIKE '%".$value_willing_to_relocate."%'";									
		 	...
		 	$where_condition.=$willing_to_relocate_where.") AND ";		
		...
		$where_condition.=$degree_tp_where." AND ";
		...
		$security_clearance_where.="(b.security LIKE '%".$value_security_clearance."%'";									
		 	...
		 	$where_condition.=$security_clearance_where.") AND ";		
		...
		$where_condition.=$year_experience_qus.")AND ";	
      ...
      $pagination_sql=  $_SESSION['pagination_sql'];
        ...
        $query12=mysql_query($pagination_sql);
     $total_rows=mysql_num_rows($query12);
     ...
     $query123="SELECT a.id,a.email,b.reg_id,b.first_name,b.middel_name,b.phone_no,b.state,b.city,b.postal_code,b.employment_type,b.work_authorization,b.comment,b.relocate,b.security,b.searchable,b.last_name,b.salary,b.hourly_rate,b.experience,b.position,e.comapnay,
     c.city,d.skill_name,d.year_exp,f.resume_data
		 FROM sedna_operator_request_consultants_reg a
		LEFT JOIN sebna_profile_tbl b ON a.id = b.reg_id
		LEFT JOIN sedna_education c ON a.id = c.reg_id
		LEFT JOIN sedna_skills d ON a.id = d.reg_id
		LEFT JOIN sebna_work_experience e ON a.id = e.reg_id
		LEFT JOIN sedna_resume f ON a.id = f.consultant_id
		WHERE $where_condition a.status='1' and b.searchable='yes' group by a.id ";				
	...
	$query12=mysql_query($pagination_sql);
     $total_rows=mysql_num_rows($query12);
		...
		$result=mysql_query($query) or die(mysql_error());
		//$num_rows = mysql_num_rows($result_p);
        ...
        <form method="post" action="?page=employer&page_no=1" >
		<div class="row ">
		<div class="col-md-1">
		<div class="input">
		...
		<div class="input">
		<div class="form-group form-md-line-input form-md-floating-label has-success">
		<input name="keyword_search" value="<?php if($_SESSION['string_key_word']!=''){ echo $_SESSION['string_key_word']; } ?>"  class="form-control <?php if($_SESSION['string_key_word']!=''){ echo "edited"; } ?>" type="text">
		...
		<ul class="two-btn">
		<li><li ><input class="btn btn-white-1 btn-custom btn-rounded waves-effect waves-light" id="configreset"  value="Reset" type="reset"><!--<a href="#">Reset</a>--></li>
		...
		<div class="col-md-9">
		<h3 class="block-head"> <?php echo $search_string_full; ?> <!--tester Quality: 0 years Coventry health QA...--> <!--<span class="fr"><a href="#"><input class="clear" name="clear" value="Save Search" type="button"></a></span>-->
		</h3>
        ...
        {
		while($fetch_result=mysql_fetch_assoc($result))
		{
            //echo "<pre>";
        ...
        <a href="#" class="f14"><i class="fa fa-users"></i></a>
